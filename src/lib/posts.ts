...
export async function getPostBySlug(slug: string) {
  ...
  if (!(postData?.data as any).post) return { post: undefined };

  const post = [(postData?.data as any).post].map(mapPostData)[0];
...
export async function getAllPosts(options: Options = {}) {
  ...
  const posts = (data?.data as any).posts.edges.map(({ node = {} }: any) => node);
